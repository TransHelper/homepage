...
const discordLink = siteConfig.social.discord
  ? `https://discord.gg/${siteConfig.social.discord}`
  : undefined
      ...
      script: [
        {
          ...
          innerHTML: JSON.stringify({
            ...
            '@type': 'Organization',
            name: 'Trans-Helper',
            url: 'https://trans-helper.org',
            description:
              'Building practical open-source software and infrastructure for the transgender community.',
            logo: 'https://trans-helper.org/favicon.png',
            sameAs: [
              'https://github.com/Trans-Helper',
            ],
          }),
        },
      ],
...
<template>
  <div class="relative min-h-screen overflow-hidden bg-zinc-50 font-sans text-zinc-900 dark:bg-[#0a0a12] dark:text-white">
    ...
    <a href="#main-content" class="sr-only focus:not-sr-only focus:fixed focus:z-50 focus:left-4 focus:top-4 focus:rounded-full focus:bg-fuchsia-500 focus:px-4 focus:py-2 focus:text-sm focus:font-semibold focus:text-zinc-950">
      Skip to content
    </a>
    ...
    <nav class="relative z-10 mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
      <div class="flex items-center gap-2">
        <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-fuchsia-500/20">
          <svg class="h-5 w-5 text-fuchsia-500 dark:text-fuchsia-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
          </svg>
        </div>
        <NuxtLink :to="localePath('/')" class="text-lg font-bold tracking-tight"><span class="text-fuchsia-500 dark:text-fuchsia-400">Trans-</span>Helper</NuxtLink>
      </div>
    ...
    <div v-if="navOpen" class="relative z-10 border-t border-zinc-200 bg-zinc-50 px-6 pb-6 md:hidden dark:border-zinc-900 dark:bg-[#0a0a12]">
    ...
    <footer class="relative z-10 border-t border-zinc-200 px-6 py-12 dark:border-zinc-900">
      <div class="mx-auto flex max-w-6xl flex-col items-center gap-8 sm:flex-row sm:justify-between">
        <div class="text-xs text-zinc-500 dark:text-zinc-700">
          &copy; {{ new Date().getFullYear() }} Trans-Helper. {{ $t('footer.copyright') }}
        </div>
        <div class="flex items-center gap-6">
          <a
            href="https://github.com/Trans-Helper"
            target="_blank"
            rel="noopener noreferrer"
