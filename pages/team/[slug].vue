...
useHead(() => ({
  title: `${member.value?.name || ""} — TransHelper`,
  script: member.value
    ? [
        {
...
<template>
  <div v-if="member" class="relative z-10 mx-auto max-w-4xl px-6 py-32">
    ...
    <div
      class="flex animate-fade-up flex-col items-center text-center sm:flex-row sm:items-start sm:text-left"
      ...
      <div class="mt-6 sm:mt-0 sm:ml-10">
        ...
        <div v-if="socialLinks.length" class="mt-5 flex flex-wrap justify-center gap-3 sm:justify-start">
          ...
          >
            ...
            <span
              class="max-w-0 overflow-hidden whitespace-nowrap transition-all duration-300 group-hover:ml-1.5 group-hover:max-w-32"
              >{{ link.label }}</span
            >
          </a>
        </div>
    ...
    <div
      class="mt-16 border-t border-zinc-200 pt-12 dark:border-zinc-800 animate-fade-up"
      style="animation-delay: 0.2s"
    >
      <!-- eslint-disable-next-line vue/no-v-html -->
      <div
        class="prose prose-zinc max-w-none dark:prose-invert prose-code:rounded prose-code:bg-zinc-100 prose-code:px-1.5 prose-code:py-0.5 prose-code:text-zinc-800 prose-code:before:content-none prose-code:after:content-none dark:prose-code:bg-zinc-800 dark:prose-code:text-zinc-200"
        v-html="member.bio"
      />
    </div>
  </div>
